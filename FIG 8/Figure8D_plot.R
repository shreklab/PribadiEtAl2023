...
# Figure 8D_plot: Generate effects contrast plot for changes to the Predator vs. Control off-lawn laying response delta.
...
ph = ph[[mdl]]
ph = ph[c("predator_wt - control_wt",
          "predator_dop1234 - control_dop1234",
          "predator_dop1 - control_dop1",
          "predator_dop2 - control_dop2",
          "predator_dop3 - control_dop3",
          "predator_dop4 - control_dop4"),]
...
if(plotscale == "linear"){
  ...
  xlims=bestupperlim(c(-5,1)*limitsOverage+range(deltas),mod=5,digits=1)
...
yticks=sort(unique(as.numeric(dd$Genotype)))
#ylabs=levels(dd$Genotype)
ylabs=c(levels(dd$Genotype)[1:2],levels(dd$Genotype)[c(-1,-2)])
