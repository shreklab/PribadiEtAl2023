...
# Figure 7B_plot: Generate plot for Fig 7b Poff with predator exposure, cat2 mutation, & exogenous DA
...
data.predator$x = as.numeric(interaction(data.predator$Genotype,data.predator$Treatment))+ 2*plotjitter + runif(nrow(data.predator),-plotjitter,plotjitter)
exemplar.predator$x = as.numeric(interaction(exemplar.predator$Genotype,exemplar.predator$Treatment))

data.control$x = as.numeric(interaction(data.control$Genotype,data.control$Treatment)) - 2*plotjitter + runif(nrow(data.control),-plotjitter,plotjitter)
exemplar.control$x = as.numeric(interaction(exemplar.control$Genotype,exemplar.control$Treatment))

### Plot limits ##
xlims=c(-1,1)+range(as.numeric(interaction(dd$Genotype,dd$Treatment)))
xticks=sort(unique(as.numeric(interaction(dd$Genotype,dd$Treatment))))
xlabs=levels(interaction(dd$Genotype,dd$Treatment))
ylims=c(0,100)
