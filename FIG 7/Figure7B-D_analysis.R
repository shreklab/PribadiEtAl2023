...
# Figure 7B-D_analysis: Analysis of eggs with predator exposure, cat2 mutation, & exogenous DA
...
d$Genotype = factor(d$Genotype,
                      levels=c("wt","cat2(e1112)"))
d$Treatment = factor(d$Treatment,levels=c("VEH","DA"))
######### DATA PROCESSING ######### 
## Binomial Aggregate Data: # eggs off, on, and total by arena
dd = aggregate(off ~ Arena*Condition*Genotype*Treatment,d,sum) # compute the number of eggs off lawn by arena
dd$on = aggregate(1-off ~ Arena*Condition*Genotype*Treatment,d,sum)$`1 - off` # compute the number of eggs on lawn by arena
...
dd$MeanDistance = aggregate(DistCenterMm ~ Arena*Condition*Genotype*Treatment,d,mean)$DistCenterMm # Mean distance from center of the arena
## Lower 25% Quartile
dd$LowerDistance = aggregate(DistCenterMm ~ Arena*Condition*Genotype*Treatment,d,quantile,probs=0.25)$DistCenterMm
## Upper 75% Quartile
dd$UpperDistance = aggregate(DistCenterMm ~Arena*Condition*Genotype*Treatment,d,quantile,probs=0.75)$DistCenterMm 
## Coefficient of Variation, Distance From center
dd$SDdist = aggregate(DistCenterMm ~ Arena*Condition*Genotype*Treatment,d,sd)$DistCenterMm # Standard Deviation
...
######### MODELS #########
mdls = list(
  pOff = glm(cbind(off,on) ~ Condition*Genotype*Treatment,dd,family=binomial),
  eggsPerWorm = lm(eggsPerWorm ~ Condition*Genotype*Treatment,dd),
  MeanDistance = lm(MeanDistance ~ Condition*Genotype*Treatment,dd),
  LowerDistance = lm(LowerDistance ~ Condition*Genotype*Treatment,dd),
  UpperDistance = lm(UpperDistance ~ Condition*Genotype*Treatment,dd),
  CVdist = lm(CVdist ~ Condition*Genotype*Treatment,dd)
)
...
exemplarData = expand.grid(Condition = levels(dd$Condition),Genotype=levels(dd$Genotype),Treatment=levels(dd$Treatment))
# Model Matrix
X=model.matrix(~Condition*Genotype*Treatment,exemplarData)
rownames(X) = levels(interaction(exemplarData$Condition,exemplarData$Genotype,exemplarData$Treatment,sep = "_"))
...
for(i in names(exemplarData)){
  ...
  if(i=="pOff"){ # also compute the linear scale inverse logit
    ...
    exemplarData[[i]] = data.frame(
      ...
      Treatment = exemplarData.old$Treatment,
      estimate = sht$test$coefficients,
      se = sht$test$sigma,
      lwr = ciht$confint[,"lwr"],
      upr = ciht$confint[,"upr"],
      ...
      Narenas = as.numeric(table(interaction(dd$Condition,dd$Genotype,dd$Treatment)))
    )
