...
# Figure 7D_plot: Generate effects contrast plot for changes to the Predator vs. Control off-lawn laying response delta.
...
# In this case, we care about posthocs (fold changes) in between predator & mock.

ph = ph[[mdl]]
ph = ph[c("predator_wt_VEH - control_wt_VEH",
          "predator_cat2(e1112)_VEH - control_cat2(e1112)_VEH",
          "predator_wt_DA - control_wt_DA",
          "predator_cat2(e1112)_DA - control_cat2(e1112)_DA"),]
...
if(plotscale == "linear"){
  ...
  xlims=bestupperlim(c(-3,1)*limitsOverage+range(deltas),mod=5,digits=1)
...
yticks=sort(unique(as.numeric(interaction(dd$Genotype,dd$Treatment))))
ylabs=levels(interaction(dd$Genotype,dd$Treatment,sep="_"))
...
if(plotscale=="linear"){
  xlab="Fold change in the Odds Ratio (P(off lawn)/P(on lawn)), predator vs. mock"
}else if(plotscale == "log"){
  xlab="Log fold change in the Odds Ratio (P(off lawn)/P(on lawn)), predator vs. mock"
}else if(plotscale == "log2"){
  xlab="Log2 fold change in the Odds Ratio (P(off lawn)/P(on lawn)), predator vs. mock"
}
