...
# Figure 5B_plot: Generate plot for Fig 5B Poff in control vs. predator, with rest periods.
...
data.predator$x = as.numeric(data.predator$RestPeriod) + 2*plotjitter + runif(nrow(data.predator),-plotjitter,plotjitter)
exemplar.predator$x = as.numeric(exemplar.predator$RestPeriod)

data.control$x = as.numeric(data.control$RestPeriod) - 2*plotjitter + runif(nrow(data.control),-plotjitter,plotjitter)
exemplar.control$x = as.numeric(exemplar.control$RestPeriod)

### Plot limits ##
xlims=c(-1,1)+range(as.numeric(dd$RestPeriod))
xticks=sort(unique(as.numeric(dd$RestPeriod)))
xlabs=levels(dd$RestPeriod)
ylims=c(0,100)
