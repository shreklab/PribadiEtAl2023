...
# Figure 5B-C_analysis: Analysis of eggs after predator exposure and 1,2,or 24 hour rest.

rm(list=ls())  # for console work
graphics.off() # for console work

wd="~/../Dropbox/chalasanilabsync/mrieger/Manuscripts/PribadiEtAl-2022/FINALv2/Figures/FIG 5/"
dataSheet = "Figure5B-C"
rawdata = "../../SourceData/RawData.xlsx"
plannedposthoc = "../../SourceData/PostHocComparisons.xlsx"
posthocSheet = "Figure5B-C"
alpha=0.05
...
d$RestPeriod = factor(d$RestPeriod,
                      levels=c("1HR","2HRS","24HRS"))
######### DATA PROCESSING ######### 
## Binomial Aggregate Data: # eggs off, on, and total by arena
dd = aggregate(off ~ Arena*Condition*RestPeriod,d,sum) # compute the number of eggs off lawn by arena
dd$on = aggregate(1-off ~ Arena*Condition*RestPeriod,d,sum)$`1 - off` # compute the number of eggs on lawn by arena
...
dd$MeanDistance = aggregate(DistCenterMm ~ Arena*Condition*RestPeriod,d,mean)$DistCenterMm # Mean distance from center of the arena
## Lower 25% Quartile
dd$LowerDistance = aggregate(DistCenterMm ~ Arena*Condition*RestPeriod,d,quantile,probs=0.25)$DistCenterMm
## Upper 75% Quartile
dd$UpperDistance = aggregate(DistCenterMm ~ Arena*Condition*RestPeriod,d,quantile,probs=0.75)$DistCenterMm 
## Coefficient of Variation, Distance From center
dd$SDdist = aggregate(DistCenterMm ~ Arena*Condition*RestPeriod,d,sd)$DistCenterMm # Standard Deviation
...
######### MODELS #########
mdls = list(
  pOff = glm(cbind(off,on) ~ Condition*RestPeriod,dd,family=binomial),
  eggsPerWorm = lm(eggsPerWorm ~ Condition*RestPeriod,dd),
  MeanDistance = lm(MeanDistance ~ Condition*RestPeriod,dd),
  LowerDistance = lm(LowerDistance ~ Condition*RestPeriod,dd),
  UpperDistance = lm(UpperDistance ~ Condition*RestPeriod,dd),
  CVdist = lm(CVdist ~ Condition*RestPeriod,dd)
)
...
exemplarData = expand.grid(Condition = levels(dd$Condition),RestPeriod=levels(dd$RestPeriod))
# Model Matrix
X=model.matrix(~Condition*RestPeriod,exemplarData)
rownames(X) = levels(interaction(exemplarData$Condition,exemplarData$RestPeriod,sep = "_"))
...
for(i in names(exemplarData)){
  ...
  if(i=="pOff"){ # also compute the linear scale inverse logit
    ...
    exemplarData[[i]] = data.frame(
      ...
      RestPeriod = exemplarData.old$RestPeriod,
      estimate = sht$test$coefficients,
      se = sht$test$sigma,
      lwr = ciht$confint[,"lwr"],
      upr = ciht$confint[,"upr"],
      ...
      Narenas = as.numeric(table(interaction(dd$Condition,dd$RestPeriod)))
    )
