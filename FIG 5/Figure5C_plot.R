...
# Figure 5c_plot: Generate effects contrast plot for rest periods, provides segue for subsequent slides on the magnitude changes
# in predator vs. control in different conditions:
...
ph = ph[[mdl]]
ph = ph[c("predator_1HR - control_1HR",
          "predator_2HRS - control_2HRS",
          "predator_24HRS - control_24HRS"),]
...
ylims=c(-1,1)+range(as.numeric(dd$RestPeriod))
yticks=sort(unique(as.numeric(dd$RestPeriod)))
ylabs=levels(dd$RestPeriod)
