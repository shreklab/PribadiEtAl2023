...
# Figure 1B-C_plot: Generate plot for Figure 1C (scatters of pOff)
...
cls=c("#5CB3E5","#E6A126","#059E74","#D36228","#CC79A7")
...
d$x = as.numeric(interaction(d$Predator)) # just for robustness across code, other figures have interaction term labels
d$cols=cls[d$x]
exemplarData = exemplarData[[mdl]] # overwrite with just the table we need
exemplarData$x = 1:length(levels(interaction(d$Predator))) #"interaction" will already be the rownames


xlims=c(-1,1)+range(d$x)
xticks=exemplarData$x
xlabs=levels(interaction(d$Predator))
ylims=bestupperlim(ylimscale*range(d[,mdl]),mod = 10,digits = 2)
yticks=seq(ylims[1],ylims[2],by=0.5)
...
points(d$x+runif(nrow(d),min=-plotjitter,max=plotjitter),
       d$norm_log2ratio,col=d$cols,pch=19)
