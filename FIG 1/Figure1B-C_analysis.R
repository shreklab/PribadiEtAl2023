...
# Figure 1B-C_analysis: Analysis of egg distributions in presence of predators (related to Figure 1B & C)
...
dd = aggregate(off ~ Arena*Strain*Predator,d,sum) # compute the number of eggs off lawn by arena
dd$on = aggregate(1-off ~ Arena*Strain*Predator,d,sum)$`1 - off` # compute the number of eggs on lawn by arena
...
dd$MeanDistance = aggregate(DistCenterMm ~ Arena*Strain*Predator,d,mean)$DistCenterMm # Mean distance from center of the arena
## Lower 25% Quartile
dd$LowerDistance = aggregate(DistCenterMm ~ Arena*Strain*Predator,d,quantile,probs=0.25)$DistCenterMm
## Upper 75% Quartile
dd$UpperDistance = aggregate(DistCenterMm ~ Arena*Strain*Predator,d,quantile,probs=0.75)$DistCenterMm 
## Coefficient of Variation, Distance From Edge
dd$SDdist = aggregate(DistCenterMm ~ Arena*Strain*Predator,d,sd)$DistCenterMm # Standard Deviation
...
######### MODELS #########
mdls = list(
  pOff = glm(cbind(off,on) ~ Predator,dd,family=binomial),
  eggsPerWorm = lm(eggsPerWorm ~ Predator,dd),
  MeanDistance = lm(MeanDistance ~ Predator,dd),
  LowerDistance = lm(LowerDistance ~ Predator,dd),
  UpperDistance = lm(UpperDistance ~ Predator,dd),
  CVdist = lm(CVdist ~ Predator,dd)
)
...
exemplarData = expand.grid(Predator = levels(dd$Predator))

# Model Matrix
X=model.matrix(~Predator,exemplarData)
rownames(X) = levels(dd$Predator)
...
for(i in names(exemplarData)){
  ...
  if(i=="pOff"){ # also compute the linear scale inverse logit
    ...
    exemplarData[[i]] = data.frame(
      ...
      Narenas = as.numeric(table(dd$Predator))
    )
