...
# Figure 4B_plot: Generate plot for Fig 4B (Mean Distance from Center in filed arena)
...
ylims=bestupperlim(c(1/limitsOverage,limitsOverage)*range(dd$MeanDistance),mod=10,digits=1)
...
points(data.predator$x,
       data.predator$MeanDistance,col=data.predator$cols,pch=19)

# control data points
points(data.control$x,
       data.control$MeanDistance,col=data.control$cols,pch=19)
