...
# Figure 1-S2A-B: Analysis of egg distributions in presence of 0-5 predators (predator ratios)

rm(list=ls())  # for console work
graphics.off() # for console work

wd="~/../Dropbox/chalasanilabsync/mrieger/Manuscripts/PribadiEtAl-2022/FINALv2/Figures/FIG 1-S2/"
dataSheet = "Figure1-S2A-B"
...
d$Ratio = factor(
  d$Ratio, 
  levels=c("0:6","1:5","2:4","3:3","4:2","5:1"
  )
)

######### DATA PROCESSING ######### 
## Binomial Aggregate Data: # eggs off, on, and total by arena
dd = aggregate(off ~ Arena*Ratio,d,sum) # compute the number of eggs off lawn by arena
dd$on = aggregate(1-off ~ Arena*Ratio,d,sum)$`1 - off` # compute the number of eggs on lawn by arena
dd$total = dd$off + dd$on
dd$pct.off = (dd$off/dd$total)*100

## Average Eggs per C. elegans in arena
dd$nCel = as.numeric(str_split(as.character(dd$Ratio),":",simplify=TRUE)[,2])
dd$eggsPerWorm = dd$total/dd$nCel

## Distributional Parameters of Egg Distance from center of arena
## Mean Distance From Edge
dd$MeanDistance = aggregate(DistCenterMm ~ Arena*Ratio,d,mean)$DistCenterMm # Mean distance from center of the arena
## Lower 25% Quartile
dd$LowerDistance = aggregate(DistCenterMm ~ Arena*Ratio,d,quantile,probs=0.25)$DistCenterMm
## Upper 75% Quartile
dd$UpperDistance = aggregate(DistCenterMm ~ Arena*Ratio,d,quantile,probs=0.75)$DistCenterMm 
## Coefficient of Variation, Distance From Edge
dd$SDdist = aggregate(DistCenterMm ~ Arena*Ratio,d,sd)$DistCenterMm # Standard Deviation
...
######### MODELS #########
mdls = list(
  pOff = glm(cbind(off,on) ~ Ratio,dd,family=binomial),
  eggsPerWorm = lm(eggsPerWorm ~ Ratio,dd),
  MeanDistance = lm(MeanDistance ~ Ratio,dd),
  LowerDistance = lm(LowerDistance ~ Ratio,dd),
  UpperDistance = lm(UpperDistance ~ Ratio,dd),
  CVdist = lm(CVdist ~ Ratio,dd)
)
...
exemplarData = expand.grid(Ratio = levels(dd$Ratio))

# Model Matrix
X=model.matrix(~Ratio,exemplarData)
rownames(X) = levels(dd$Ratio)
...
for(i in names(exemplarData)){
  ...
  if(i=="pOff"){ # also compute the linear scale inverse logit
    ...
    exemplarData[[i]] = data.frame(
      ...
      Narenas = as.numeric(table(dd$Ratio))
    )
