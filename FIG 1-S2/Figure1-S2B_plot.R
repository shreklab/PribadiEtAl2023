...
# Figure 1-S2B_plot: Generate plot for Fig 1-S1B (# eggs per c elegans in different predator:prey ratios)
...
cls=c("#5CB3E5",rep("#D36228",5))
names(cls)=levels(dd$Ratio)
# This column is called Interaction for robustness with other Figures, but 
# this particular dataset is a single factor term.
dd$x = as.numeric(interaction(dd$Ratio)) # just for robustness across code, other figures have interaction term labels
dd$cols=cls[dd$x]
exemplarData = exemplarData[[mdl]] # overwrite with just the table we need
exemplarData$x = 1:length(levels(interaction(dd$Ratio))) #"interaction" will already be the rownames


xlims=c(-1,1)+range(dd$x)
xticks=exemplarData$x
xlabs=levels(interaction(dd$Ratio))
ylims=bestupperlim(c(1/limitsOverage,limitsOverage)*range(dd$eggsPerWorm),mod=10,digits=0)
yticks=seq(ylims[1],ylims[2],by=10)
